...
def classify_rudiment_events(audio_data: np.ndarray, sr: int, onsets: list[float]) -> list[dict]:
    """
    Classification engine for single beats and rudiments.
    Guarantees 1 event per single-beat sample.
    """
    from drumscript.notation_generator import constants
    classified_events = []
    ...
    for onset_time in onsets:
        ...
        # Long slice padding (1.5s) for measuring decay
        ...
        if is_metal:
            ...
            # Kicks, Snares, and Toms
            is_kick_freq = p['peak_freq'] < 105.0
            is_thump = p['lfer'] > 0.35
            
            # A kick MUST have a short decay (< 0.40s). Toms ring out longer.
            if is_kick_freq and is_thump and p['decay'] < 0.40:
                instruments.append('kick')
            elif p['hfer'] > 0.20:
                instruments.append('snare')
            else:
                # It is a Tom. We separate them by frequency.
                if p['peak_freq'] < 90.0:
                    instruments.append('low_tom')
    ...
    # Single beat guarantee for short files
