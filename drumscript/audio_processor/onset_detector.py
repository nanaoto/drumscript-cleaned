...
def main(input_audio_path: str, transcribe_full_song: bool = False):
    ...
    try:
        ...
        # This now correctly gets frame indices [4410, 22050, ...]
        ...
        # This now correctly passes frame indices
        ...
        # [FIX] Renamed variable for clarity and to fix loop bug
        classified_drum_types = predict.predict_drum_hits(features)
        ...
        # --- [START OF FIX] ---
        # Call the new, correct function 'frames_to_time' from onset_detector
        onset_times_sec = onset_detector.frames_to_time(onset_frames, sr=sr)
        # --- [END OF FIX] ---

        # Ensure we have the same number of timestamps as classified hits
        if len(onset_times_sec) != len(classified_drum_types): # Updated variable name
            print(f"Error: Mismatch in onset times ({len(onset_times_sec)}) and classified hits ({len(classified_drum_types)})")
            return 
        ...
        # --- [START OF FIX 2] ---
        # Renamed the output list to 'final_classified_events'
        # to avoid reading from and writing to the same list.
        final_classified_events = [] 
        for i in range(len(onset_times_sec)):
            time_sec = onset_times_sec[i]
            drum_types = classified_drum_types[i] # Get from the original list
            
            # Only add events that actually have drum hits (not empty classifications)
            if drum_types: 
                final_classified_events.append({ # Add to the new list
                    'time': time_sec,
                    'drums': drum_types
                })
        # --- [END OF FIX 2] ---
                
        print(f"Successfully combined {len(final_classified_events)} detected drum events with timestamps.")
        ...
        score_builder.build_and_export_drum_score(
            detected_events=final_classified_events, # Use the new, correct list
