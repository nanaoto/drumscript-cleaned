...
# Purpose: Test script to generate a clean drum score using a centralized constants map.
...
def generate_drum_score(events, tempo=120, output_filename="drum_test_score"):
    ...
    for i, group in enumerate(grouped_events):
        ...
        if i < len(grouped_events) - 1:
            duration_qn = (grouped_events[i+1][0]['onset_time_seconds'] - onset_time_sec) * (tempo / 60.0)
        else:
            duration_qn = 0.5
    ...
    try:
        ...
        print(f"ERROR: MuseScore executable not found at '{musescore_path}'.")
    except subprocess.CalledProcessError as e:
        print(f"ERROR: MuseScore failed to convert the file: {e.stderr}")
