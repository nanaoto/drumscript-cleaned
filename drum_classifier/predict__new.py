...
DRUM_METADATA = {
    ...
    'hi_hat_open': {
        'midi_pitch': 46,
        'note_head_type': 'x-open',
        'staff_position': 'F#3',
        'display_name': 'Hi-Hat (Open)'
    },
    # --- NEW: Add metadata for our new tom instruments ---
    'low_tom': {
...
# --- UPDATED: Make the kick drum rule more specific ---
KICK_SPECTRAL_CENTROID_MAX = 800  # Kicks are very low frequency. Let's cap them here.

SNARE_CENTROID_MIN = 1000
SNARE_CENTROID_MAX = 4000
SNARE_ZCR_MIN = 0.09

# --- NEW: Define a rule space for our Low Tom ---
# It should live between the kick and the snare.
...
# --- NEW: Add a refractory period for toms ---
# Toms have long sustains, so let's give them a long cooldown.
TOM_REFRACTORY_PERIOD_S = 1.0
...
def predict_drum_hits(onset_features: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # --- NEW: Add a tracker for the last tom hit ---
    last_tom_time = -1.0
    ...
    for onset in onset_features:
        ...
        if last_open_hihat_time > 0 and (onset['onset_time'] - last_open_hihat_time) < OPEN_HIHAT_REFRACTORY_PERIOD_S:
            print(f"\n--- Ignoring Onset at {onset['onset_time']:.2f}s (within refractory period of open hi-hat) ---")
            continue
        # --- NEW: Check if we are inside the sustain of a recently hit tom ---
